...
export function createStackOptions<T extends StackPayload>(
	payload: T,
	options?: Partial<FocusableOptions<T>>
): FocusableOptions<T> {
	return {
		id: DefinedFocusable.Stack,
		payload,
		displayName: `Stack: ${payload.stackId}`,
		tags: ['stack', `stack-${payload.stackId}`],
		...options
	};
}
...
export interface FocusableOptions<T extends Payload> {
	id?: DefinedFocusable;
	parentId?: DefinedFocusable;
	payload?: T; // Type-safe arbitrary data
	skip?: boolean;
	list?: boolean;
	// Event handlers
...
export class FocusManager {
	...
	register<T extends object>(options: FocusableOptions<T>, element: HTMLElement) {
		...
		// if (
		// 	element.classList.toString().includes('stack-view') ||
		// 	element.classList.toString().includes('file') ||
		// 	element.classList.toString().includes('branch')
		// ) {
		...
		// 		logicalId,
		// 		parentElement,
		// 		parentChildren: parentMeta?.children.slice()
		// 	});
		// }
